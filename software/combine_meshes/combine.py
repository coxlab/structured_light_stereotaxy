...
# meshFiles = ['%s/skullInSkull_simple.obj' % baseDir,
# '%s/hatInSkull_simple.obj' % baseDir, '%s/finalInSkull_simple.obj' %
# baseDir]
meshFiles = ['%s/%s' % (baseDir, f) for f in os.listdir(baseDir)
             if '.obj' in f]
# textureFiles = ['%s/skullInSkull.png' % baseDir, '%s/hatInSkull.png' % baseDir]
# textureFiles = ['%s/skullInSkull.png' % baseDir, '%s/hatInSkull.png' %
# baseDir, '%s/finalInSkull.png' % baseDir]
textureFiles = ['%s/%s' % (baseDir, f) for f in os.listdir(baseDir)
                if '.png' in f]
...
currentX = 0
for t in textures:
    combinedTexture.paste(t.crop((0, 0, t.size[0], t.size[1])), (
        currentX, maxHeight - t.size[1], currentX + t.size[0], maxHeight))
    currentX += t.size[0]
...
for (meshFile, textureMapping) in zip(meshFiles, textureMappings):
    ...
    if len(mesh.texCoords) != len(mesh.vertices):
        ...
        raise Exception(
            "len(texCoords) != len(vertices) for mesh: %s" % meshFile)
    ...
    combinedMesh.faces = vstack((combinedMesh.faces, mesh.faces + nPoints))
                                # accounting for previous vertices and
                                # texCoords
    xoffset, xscale, yoffset, yscale = textureMapping
    newTexCoords = make_homogeneous(mesh.texCoords) * matrix(
        [[xscale, 0, 0], [0, yscale, 0], [xoffset, yoffset, 1]])

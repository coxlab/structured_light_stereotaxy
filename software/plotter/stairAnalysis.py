    ...
    while (tries < maxTries):
        ...
        if len(clusters) != NClusters:
            ...
            if NClusters == 1:
                ...
                raise ValueError(
                    "process_file could not find >1 cluster in file:%s" % filename)
...
def rotate_about_x(points, angle, centroid):
    for i in xrange(len(points['x'])):
        x, y, z = points['x'][i], points['y'][i] - centroid[
            'y'], points['z'][i] - centroid['z']
        nx = x
...
def rotate_about_y(points, angle, centroid):
    for i in xrange(len(points['x'])):
        x, y, z = points['x'][i] - centroid['x'], points['y'][
            i], points['z'][i] - centroid['z']

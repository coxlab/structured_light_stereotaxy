...
baseSP = subprocess.Popen("python %s %s" % (
    zoomViewBin, baseTextureFilename), shell=True, stdout=baseSO)

# find ref points in remap
remapPtsFilename = 'remapPts'
remapSO = file(remapPtsFilename, 'w')
subprocess.Popen("python %s %s" % (
    zoomViewBin, remapTextureFilename), shell=True, stdout=remapSO).wait()
baseSP.wait()
baseSO.close()
remapSO.close()

basePtsXY = pylab.loadtxt(basePtsFilename)
basePtsUV = basePtsXY / (float(baseTexture.shape[1]), float(
    baseTexture.shape[0]))
...
remapPtsUV = remapPtsXY / (float(remapTexture.shape[1]), float(
    remapTexture.shape[0]))
...
remapObj.vertices = vector.apply_matrix_to_points(
    pylab.matrix(T), remapObj.vertices)[:, :3]

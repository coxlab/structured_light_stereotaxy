...
def calculate_rigid_transform(fromPts, toPts, p0=zeros(6, dtype=float64), **kwargs):
    ...
    def residuals(p):
        ...
        print "sse:", sum(r ** 2)
        return r
    # fit_it = lambda p: ravel((toPts - (fromPts *
    # transform_to_matrix(*p)))[:,:3])
    fit_it = residuals
    # ftol : relative error desired in sum of square
    # xtol : relative error desired in the approximate solution
    bestT, cov_x, infodict, mesg, ier = optimize.leastsq(
        fit_it, p0, maxfev=14000, full_output=True, **kwargs)

...
class OBJ:
    ...
    def load_obj(self, filename):
        ...
        # zf[:,1,:] =
        # numpy.transpose(numpy.vstack((self.faces['t1'],self.faces['t2'],self.faces['t3'])))
        # - 1
        ...
        if len(self.faces) != 0:
            if len(self.texCoords) != 0:
                if len(self.normals) != 0:
                    for f in self.faces:
                        # f p1/t1/n1 ...
                        o.write("f %i/%i/%i %i/%i/%i %i/%i/%i\n" % (f[0][0] + 1, f[2][0] + 1, f[1][0] + 1, f[0][1] +
                                1, f[2][1] + 1, f[1][1] + 1, f[0][2] + 1, f[2][2] + 1, f[1][2] + 1))
                else:
                    for f in self.faces:
                        # f p1/t1 ...
                        o.write("f %i/%i %i/%i %i/%i\n" % (f[0][0]
                                + 1, f[2][0] + 1, f[0][1] + 1, f[2][1] + 1, f[0][2] + 1, f[2][2] + 1))
            else:
                for f in self.faces:
                    # f p1 ...
                    o.write(
    ...
    def save_as_pov(self, outputFile):
        ...
        # o.write('texture_list { 1 texture {pigment{uv_mapping image_map{jpeg
        # "texture.jpg"}}}}')
    ...
    def get_normals(self, u, v, faceIndices=None):
        ...
        for fi in faceIndices:
            ...
            normals.append(
                vector.calculate_normal(self.vertices[self.faces[fi][0][0]],
    ...
    def get_average_normal(self, position, maxDistance):
        ...
        for f in closeFaces:
            normal += numpy.array(
                vector.calculate_normal(self.vertices[f[0][0]],
                                        self.vertices[f[0][1]],
                                        self.vertices[f[0][2]]))
        normal /= nFaces
...
if __name__ == '__main__':
    ...
    for i in xrange(len(inFaces)):
        ...
        ax.scatter([p1[0], p2[0], p3[0]], [p1[1], p2[1], p3[1]], [
                   p1[2], p2[2], p3[2]], c='b')
        ax.plot([p1[0], p2[0], p3[0], p1[0]], [p1[1], p2[1], p3[1],
                p1[1]], [p1[2], p2[2], p3[2], p1[2]], c='b')

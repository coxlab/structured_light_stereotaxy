...
class Bezier:
    ...
    def test_intersection(self, other):
        ...
        while (err > 0.000001): # if error decreased less than threshold, than return
            ...
            # test if any of those those segments intersect (using seg_intersect)
    ...
    def at_t(self,t):
        return (self.a[0]*t**3 + self.b[0]*t**2 + self.c[0]*t + self.d[0],
...
class PanZoom:
    ...
    def setup_projection(self):
        ...
        glLoadIdentity ();
        glOrtho (self.offset[0], self.zoom+self.offset[0], self.offset[1]+self.zoom, self.offset[1], 0, 1) # so gl agrees with texture coords
            ...
            if len(lines) > 1:
                ...
                for i in xrange(len(lines)-1):
                    for j in xrange(len(lines)-i-1):
                        intersections.append(lines[i].test_intersection(lines[i+j+1]))
                lines = []
            points = []
            for i in intersections:
                if i[0]:
                    c = _defaultColors[len(points) % len(_defaultColors)]
                    # add intersection to items, maybe make these crosses NOT points

...
tool = 'Select'
toolButtonIDs = [['Select', 0], ['Rotate', 0], ['Translate', 0], [
...
def view_select(button, state, x, y):
    ...
    glLoadIdentity()
    gluLookAt(
        trans[0], trans[1], viewRadius, trans[0], trans[1], trans[2], 0, 1, 0)
...
def view_rotate_motion(x, y):
    ...
    global rotQuaternion
    rotQuaternion = quaternion.fromEuler(
        (y - viewDown[1]) * 0.01, (x - viewDown[0]) * 0.01, 0.) * rotQuaternion
...
def display_select():
    ...
    if len(selectData) == 3:
        ...
        points.sort(0)
        dists = [sqrt(
            sum((points[i - 1] - points[i]) ** 2)) for i in xrange(len(points))]
        r1 = numpy.product(dists) / sqrt(2 * dists[0] ** 2 * dists[1] ** 2 + 2 * dists[1] ** 2 * dists[2] ** 2 +
                                         2 * dists[2] ** 2 * dists[0] ** 2 - dists[0] ** 4 - dists[1] ** 4 - dists[2] ** 4)
        print "Radius of arc:", r1
    glClearColor(0.2, 0.2, 0.2, 1.)
    glClear(GL_COLOR_BUFFER_BIT)
    glColor(1., 1., 1., 1.)
    display_table(selectData, selectHeader, selectColWidths,
                  selectRowHeight, 8, 26, pointColorMap)
    w = glutGet(GLUT_WINDOW_WIDTH)
    h = glutGet(GLUT_WINDOW_HEIGHT)
    glColor(1., 1., 1., 1.)
    glBegin(GL_LINES)
...
for i in range(len(toolButtonIDs)):
    toolButtonIDs[i] = [toolButtonIDs[i][0], glutCreateSubWindow(
        toolWinID, x, y, dW, dH)]
    x += dW + 10
...
# selection info
selectWinID = glutCreateSubWindow(
    mainWinID, winWidth - 236, 51, 236, winHeight - 51)
glutDisplayFunc(display_select)
glutMouseFunc(select_clicked)
glutKeyboardFunc(keyboard_main)

# point display
viewWinID = glutCreateSubWindow(
    mainWinID, 0, 51, winWidth - 237, winHeight - 51)
